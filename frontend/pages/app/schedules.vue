<template>
    <v-container fluid class="fill-height align-start">
        <v-card width="100%" flat>
            <v-card-title>Programaciones</v-card-title>
            <v-card-text>
                <v-data-table :items="items" :headers="headers" :items-per-page="perPage"
                    @update:itemsPerPage="v => perPage = v">

                    <template v-slot:item.option="{ item }">
                        <v-btn class="my-2 bg-grey-darken-4" icon="mdi-pencil" @click="getSchedule(item.id)"></v-btn>
    ...
    <v-dialog v-model="createDialog" max-width="500">
        <v-card>
            ...
            <v-card-text>
                <v-form @submit.prevent="createSchedule">
                    <v-row>
                        <v-col cols="12">
    ...
    <v-dialog v-model="updateDialog" max-width="500">
        <v-card>
            ...
            <v-card-text>
                <v-form @submit.prevent="updateSchedules">
                    <v-row>
                        <v-col cols="12">
                            <v-text-field v-model="form.fullName" hide-details="auto" label="Full name"></v-text-field>
                        </v-col>
    ...
    <v-dialog v-model="deleteDialog" max-width="500">
        <v-card>
            ...
            <v-card-text class="text-center py-6">
                <p class="text-subtitle-1 mb-6 text-center">¿Are you sure you want to delete the schedule?</p>
    ...
    <v-dialog v-model="searchDialog" max-width="500">
        <v-card>
            ...
            <v-card-text>
                <p class="text-subtitle-2 pb-2">Search schedule by name</p>
...
export default {
    ...
    watch: {
        async perPage(value) { await this.getSchedules(); }
    },
    async beforeMount() { await this.getSchedules(); },
    methods: {
        async getSchedules() {
            try {
                const { items } = await $fetch('/api/schedules/', {
                    query: {
                        search: this.search,
                        page: this.page,
        ...
        async getSchedule(id) {
            try {
                const schedule = await $fetch(`/api/schedules/${id}`, {
                    method: 'GET'
                })
                this.form = schedule
        ...
        async createSchedule () {
            try {
                const response = await $fetch('/api/schedules/signup', {
                    method: 'POST',
                    body: this.form
                })
        ...
        async updateSchedules() {
            try {
                const response = await $fetch(`/api/schedules/${this.form.id}`, {
                    method: 'PUT',
                    body: this.form
                })
                await this.getSchedules()
